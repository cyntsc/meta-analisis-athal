...
# Method include testing the different agglomerative coefficients 
# Based on the work of Nguyen, QH., Le, DH. Improving existing analysis pipeline to identify and analyze cancer driver genes using multi-omics data. Sci Rep 10, 20521 (2020). https://doi.org/10.1038/s41598-020-77318-1https://github.com/huynguyen250896/drivergene/blob/master/Data_n_SourceCode/2.Association-significantgenes-and-hubgenes.R 
# Repo:  huynguyen250896 / drivergene 
# FILE: 1.WGCA-STEPbySTEP.R
...
## Called the the hierarchical clustering function
...
#plot(geneTree, xlab="", sub="", main="Gene clustering on TOM-based dissimilarity (A thaliana infected / average method)",
#     cex=0.3)   # sub="Signed-Ntw (average)" , labels= FALSE, hang=0.04
plot(geneTree, xlab="", sub="", main = "Gene clustering on TOM-based dissimilarity (A thaliana infected / ward method)",
     cex = 0.3, cex.lab = 1, cex.axis = 1, cex.main = 1.2);
...
table(dynamicMods)
#write.table(table(dynamicMods), file = "results_ward/dynamicMods_Athal_Infected_MatrixE.txt", sep = ",", quote = FALSE, row.names = F)
...
write.table(sort(table(dynamicColors), decreasing = TRUE), file = "results_ward/dynamicColors_Athal_Infected_MatrixE.txt", sep = ",", quote = FALSE, row.names = F)
...
TOMplot(dissTOM, 
        ...
        main = "TOM for A.thaliana infected");
...
# Calculate eigengenes. Module eigengene is defined as the first principal component of the expression matrix of the corresponding module.
...
write.table(sort(table(mergedColors), decreasing = TRUE), file = "results_ward/mergedColors_Athal_Infected_MatrixE.txt", sep = ",", quote = FALSE, row.names = F)
...
plotDendroAndColors(geneTree, main= "Clustering of MEs at 0.90 merged \nA thaliana infected",   #ward meth
...
for (color in module_colors){
  ...
  write.table(module, paste("results_ward/module_",color, ".txt",sep=""), sep="\t", row.names=FALSE, col.names=FALSE,quote=FALSE)
}
...
for (color in module_colors){
  ...
  write.table(module, paste("results_ward/mod_merged_",color, ".txt",sep=""), sep="\t", row.names=FALSE, col.names=FALSE,quote=FALSE)
}
...
TOMplot(dissTOM, 
        ...
        main = "TOM for A.thaliana infected Merged at 0.10");
