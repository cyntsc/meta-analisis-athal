...
###
### DATA ASSUMPIONS:
### 1) RNA-Seq libraries are                             ** 8  H E A L T H Y ** 
...
athalDataHealthy <-  read.csv(here("data", "matrix_D_healthy.csv"), header=TRUE,  sep='\t')
dim(athalDataHealthy);   # 19792  x  8
## rearrange columns accordint to traits meta-data (must match)
col_order <- c("Genes", "healthy12",   "healthy12.1", "healthy18",   "healthy18.1", "healthy24",
               "healthy24.1", "healthy30",   "healthy30.1")

athalDataHealthy <- athalDataHealthy[, col_order]
## You see that genes are listed in a column named "Genes" and samples are in columns
athalDataHealthy[1:5,]

## Manipulate athalDataHealthy so it matches the format WGCNA needs
row.names(athalDataHealthy) = athalDataHealthy$Genes   ## Pull the gene names 
gene.names=rownames(athalDataHealthy)
## Pull the names according with the size grp
SubGeneNames=gene.names;

## Transpose data to set as WGCNA needs
athalDataHealthy$Genes = NULL             ## Set genes column as index row
athalDataHealthy = as.data.frame(t(athalDataHealthy)) # transpose dataset, now samples are rows and genes are columns
#dim(athalDataHealthy) 
athalDataHealthy[,1:5]
rownames(athalDataHealthy)   ## these are the sample names

## Run this to check if there are gene outliers
gsg = goodSamplesGenes(athalDataHealthy, verbose = 3)
gsg$allOK
...
if (!gsg$allOK)
   {if (sum(!gsg$goodGenes)>0)
       printFlush(paste("Removing genes:", paste(names(athalDataHealthy)[!gsg$goodGenes], collapse= ", ")));
       if (sum(!gsg$goodSamples)>0)
           printFlush(paste("Removing samples:", paste(rownames(athalDataHealthy)[!gsg$goodSamples], collapse=", ")))
       athalDataHealthy= athalDataHealthy[gsg$goodSamples, gsg$goodGenes]
       }

# TRAITMENTS BY SAMPLE (fungi-type-hpi interaction)
datTraits <- read.csv(here("data", "Traits_Healthy_sample.csv"), header=TRUE,  sep=',')   
datTraits = subset(datTraits, select = -c(sample));
datTraits[1:5,]
datTraits <- datTraits[1:(nrow(datTraits)),]
dim(datTraits)
...
table(rownames(datTraits)==rownames(athalDataHealthy)) #should return TRUE if datasets align correctly, otherwise your names are out of order

# TRAITMENTS BY HPI
datTraits2 <- read.csv(here("data", "Traits_Healthy_hpi.csv"), header=TRUE,  sep=',')   
datTraits2 = subset(datTraits2, select = -c(sample));
datTraits2[1:5,]
datTraits2 <- datTraits2[1:(nrow(datTraits2)),]
dim(datTraits2)
## form a data frame analogous to expression data that will hold the infection traits.
...
sft=pickSoftThreshold(athalDataHealthy,
  dataIsExpr = TRUE,
...
#warnings()
sft
#write.table(sft, file = "results/SFT_corr_Athal_Healthy_MatrixD.txt", sep = ",", quote = FALSE, row.names = T)
...
plot(sft$fitIndices[,1], -sign(sft$fitIndices[,3])*sft$fitIndices[,2],xlab="Soft Threshold (power)",
     ...
     type="n", main = paste("Scale independence for Athal.healthy \n (Matrix D)"));
...
abline(h=280 ,col="green");

softPower = 28 # all_infected NO Ss30 samples

# Max correlation factor = 0.8010
...
adjacency= adjacency(athalDataHealthy,type = "signed",    #specify network type
                     power = softPower);
head(adjacency)
dim(adjacency)
#adjacency[1:20]
...
plot(geneTree, xlab="Gene clusters", main="Gene clustering on TOM−based dissimilarity for A.thaliana healthy (Matrix D)", cex=0.3)   # sub="Signed-Ntw (average)" , labels= FALSE, hang=0.04
...
table(dynamicMods)
#write.table(table(dynamicMods), file = "results/dynamicMods_Athal_Healthy_MatrixD.txt", sep = ",", quote = FALSE, row.names = F)
...
#write.table(sort(table(dynamicColors), decreasing = TRUE), file = "results/dynamicColors_Athal_Healthy_MatrixD.txt", sep = ",", quote = FALSE, row.names = F)
...
plotDendroAndColors(geneTree, dynamicColors, "Dynamic Tree Cut",dendroLabels = FALSE, hang = 0.03,addGuide = TRUE, guideHang = 0.05,
                    main = "Gene dendrogram and module colors for Athal healthy \n Matrix D")

save(dynamicMods, dynamicColors, file="Athal_Healthy_DynamicMods_MatrixD.RData")
#load("Athal_Healthy_DynamicMods_MatrixD.RData")
...
MEList= moduleEigengenes(athalDataHealthy, colors= dynamicColors,
                         excludeGrey = TRUE,
                         softPower = softPower)
...
save(MEList, MEs, MEDiss, METree, file= "Athal_Healthy_Module_Identification_MatrixD.RData")
#load("Athal_Healthy_Module_Identification_MatrixD.RData")
...
plot(METree, main= "Clustering of module eigengenes for A.thaliana healthy (Matrix D)", 
...
merge = mergeCloseModules(athalDataHealthy, dynamicColors, 
...
#write.table(sort(table(mergedColors), decreasing = TRUE), file = "results/mergedColors_Athal_Healthy_MatrixD.txt", sep = ",", quote = FALSE, row.names = F)
...
TOMplot(dissTOM, 
        ...
        main = "TOM for A.thaliana healthy Merged at 0.10 (Matrix D)");
...
MEs0 = moduleEigengenes(athalDataHealthy, moduleColors)$eigengenes  # Calculates module eigengenes (1st principal component) of modules in a given single dataset
MEs = orderMEs(MEs0)
...
#par(mar= c(6, 8.5, 3, 3))
sizeGrWindow(9, 12)
par(mar= c(4, 10, 2, 1))    # margen inferior - y(izq) + margen superior - y(der)
...
labeledHeatmap(Matrix= moduleTraitCor,
               ...
               cex.text= 0.6,
               zlim= c(-1,1),
               main= paste("Module-trait relationships by sample for A thaliana healthy (Matrix D)"))
...
labeledHeatmap(Matrix= moduleTraitCor2,
               ...
               cex.text= 0.6,
               zlim= c(-1,1),
               main= paste("Module-trait by hpi for A thaliana healthy (Matrix D)"))
