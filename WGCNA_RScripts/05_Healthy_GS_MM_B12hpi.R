...
# Define variable Healthy12 containing the Healthy12 column of datTrait2
Healthy12 = as.data.frame(datTraits2$healthy12);
names(Healthy12) = "Healthy12hpi"
...
geneTraitSignificance = as.data.frame(cor(athalDataHealthy, Healthy12, use = "p"));               # Gene Significance GS is the absolute value of the correlation between the gene and the trait.
GSPvalue = as.data.frame(corPvalueStudent(as.matrix(geneTraitSignificance), nSamples));
names(geneTraitSignificance) = paste("GS.", names(Healthy12), sep="");
names(GSPvalue) = paste("p.GS.", names(Healthy12), sep="");
...
plot(    x, 
         ...
         ylab ="Gene significance for healthy_12", col = module)
...
file_name = paste("../results-data/All_Top15_GS_GO_TAIR_Annotations/Healthy_12", module, "GS_MM_genes.csv", sep = "_");
