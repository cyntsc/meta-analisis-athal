...
##                      ANALYSIS FOCUSED IN ARABIDOPSIS HEALTHY AT 24 HPI
##
###################################################################################################
...
# Define variable Healthy24 containing the Healthy24 column of datTrait2
Healthy24 = as.data.frame(datTraits2$healthy24);
names(Healthy24) = "Healthy24hpi"
...
geneTraitSignificance = as.data.frame(cor(athalDataHealthy, Healthy24, use = "p"));               # Gene Significance GS is the absolute value of the correlation between the gene and the trait.
GSPvalue = as.data.frame(corPvalueStudent(as.matrix(geneTraitSignificance), nSamples));
names(geneTraitSignificance) = paste("GS.", names(Healthy24), sep="");
names(GSPvalue) = paste("p.GS.", names(Healthy24), sep="");
...
plot(    x, 
         ...
         ylab ="Gene significance for healthy_24", col = module)
...
# For coral1:
#file_name = paste("../results-data/All_Top15_GS_GO_TAIR_Annotations/Healthy_24", module, "GS_MM_genes.csv", sep = "_");
# For magenta2:
file_name = paste("../results-data/All_Top15_GS_GO_TAIR_Annotations/Healthy_24", module, "GS_MM_genes_low.csv", sep = "_");
