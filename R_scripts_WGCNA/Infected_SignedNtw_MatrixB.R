...
###                                                       SAMPLES OF A.THALIANA MATRIX B       
###
...
athalData3 <-  read.csv(here("data", "matrix_B_infected.csv"), header=TRUE,  sep=',')
dim(athalData3);  
...
#warnings()
sft
write.table(sft, file = "results/SFT_corr_Athal_Infected_MatrixB.txt", sep = ",", quote = FALSE, row.names = T)
