...
datTraits2 = subset(datTraits2, select = -c(accesion, B_cinerea, C_higginsianum, S_sclerotium));
datTraits2[1:5,]
...
save(athalData3, datTraits, datTraits2, file="Athal_Infected_Log2TPM_SamplesAndTraits_NO_SS30.RData")
load("Athal_Infected_Log2TPM_SamplesAndTraits_NO_SS30.RData")
datTraits2 = subset(datTraits2, select = -c(X30hpi));  # I removed this label because of the removal of the Ss30 samples (reported with atypical distribution)
datTraits2[1:14,]
...
save(dissTOM, file="DissTOM_Log2TPM_14samples.RData")
load("DissTOM_Log2TPM_14samples.RData")
...
plot(geneTree, xlab="Gene clusters", main="Gene clustering on TOM−based dissimilarity for A.thaliana infected (signed-Ntw)", cex=0.3)                  #sub="Signed-Ntw (average)" , labels= FALSE, hang=0.04
...
save(dynamicMods, dynamicColors, file="DynamicMods_Log2TPM_14Samples.RData")
load("DynamicMods_Log2TPM_14Samples.RData")
...
save(MEList, MEs, MEDiss, METree, file= "Network_Module_Identification_14samples.RData")
#load("Network_Module_Identification_14samples.RData")
...
save(MEs, moduleLabels, moduleColors, geneTree, file= "Network_signed_merge_14samples.RData")
load("Network_signed_merge_14samples.RData")
...
TOMplot(dissTOM, 
        ...
        main = "TOM for A.thaliana infected (Merged 0.10)");
...
labeledHeatmap(Matrix= moduleTraitCor2,
               ...
               cex.text= 0.6,
               zlim= c(-1,1),
               main= paste("Module-trait by hpi for A thaliana infected"))
...
length(modNames)
module_colors = setdiff(unique(modNames), "grey")     ## exclude the grey module reserved for not assigned genes
length(module_colors)
#module_colors
for (color in modNames){
  module=SubGeneNames[which(modNames==color)]
