...
### 2) Data were normalized and standarized. Current scale is Log2(TPM)
###    a) Defined standard rules: 
###       elimination of genes in zeros in common genes thru all samples, remotion of TPM<0.1 and TMP>1000

### Here some code references, but notice were used many more: 
...
here("files", "data", "all_healthy_Log2TPM.csv");
athalDataH <-  read.csv(here("data", "all_healthy_Log2TPM.csv"), header=TRUE,  sep='\t')

dim(athalDataH);
## Pull the names according with the size grp
SubGeneNames=names(athalDataH);

## rearrange columns accordint to traits meta-data (must match)
col_order <- c("Genes", 'healthy12', 'healthy12.1', 'healthy18', 'healthy18.1', 'healthy24', 'healthy24.1', 'healthy30', 'healthy30.1')
athalDataH <- athalDataH[, col_order]
## You see that genes are listed in a column named "Genes" and samples are in columns
athalDataH[1:5,]

## Manipulate athalDataH so it matches the format WGCNA needs
row.names(athalDataH) = athalDataH$Genes   ## Pull the gene names 
#gene.names=rownames(athalDataH)

## Transpose data to set as WGCNA needs
athalDataH$Genes = NULL             ## Set genes column as index row
athalDataH = as.data.frame(t(athalDataH)) # transpose dataset, now samples are rows and genes are columns
dim(athalDataH) 
athalDataH[,1:5]
rownames(athalDataH)   ## these are the sample names

## Run this to check if there are gene outliers
gsg = goodSamplesGenes(athalDataH, verbose = 3)
gsg$allOK
...
#       printFlush(paste("Removing genes:", paste(names(athalDataH)[!gsg$goodGenes], collapse= ", ")));
#       if (sum(!gsg$goodSamples)>0)
#           printFlush(paste("Removing samples:", paste(rownames(athalDataH)[!gsg$goodSamples], collapse=", ")))
#       athalDataH= athalDataH[gsg$goodSamples, gsg$goodGenes]
#       }

## Create an object called "datTraits" that contains the trait data
datTraits <- read.csv(here("data", "Traits_H.csv"), header=TRUE,  sep=',')   
datTraits[1:5,]
...
table(rownames(datTraits)==rownames(athalDataH)) #should return TRUE if datasets align correctly, otherwise your names are out of order
...
save(athalDataH, datTraits, file="SamplesAndTraits_Healthy.RData")
load("SamplesAndTraits_Healthy.RData")
...
sft=pickSoftThreshold(athalDataH,
  dataIsExpr = TRUE,
...
plot(sft$fitIndices[,1], -sign(sft$fitIndices[,3])*sft$fitIndices[,2],xlab="Soft Threshold (power)",
     ...
     type="n", main = paste("Scale independence for A.thaliana healthy"));
...
adj= adjacency(athalDataH,type = "signed",    #specify network type
               power = softPower);
head(adjacency)
dim(adj)
#adj[1:100]
...
TOM=TOMsimilarityFromExpr(athalDataH,
                          TOMType = "signed", 
                          power = softPower);
dissTOM=1-TOM
...
plot(geneTree, xlab="Gene clusters",  
     main="Clustering tree based on the TOMdiss measure for A.thaliana healthy", 
...
MEList= moduleEigengenes(athalDataH, 
...
MEDissThres = 0.1
merge = mergeCloseModules(athalDataH, 
                          dynamicColors, 
...
nGenes = ncol(athalDataH)
nSamples = nrow(athalDataH)
## Recalculate MEs with color labels
MEs0 = moduleEigengenes(athalDataH,mergedColors)$eigengenes   #moduleColors    Calculates module eigengenes (1st principal component) of modules in a given single dataset
MEs = orderMEs(MEs0)
...
labeledHeatmap(Matrix= moduleTraitCor,
               ...
               cex.text= 0.5,
               zlim= c(-1,1),
               main= paste("Module-trait relationships for A thaliana infected"))
...
geneModuleMembership = as.data.frame(cor(athalDataH, MEs, use = "p"))
MMPvalue = as.data.frame(corPvalueStudent(as.matrix(geneModuleMembership), nSamples))

names(geneModuleMembership) = paste("MM", modNames, sep="")
names(MMPvalue) = paste("p.MM", modNames, sep="")

geneTraitSignificance = as.data.frame(cor(athalDataH, BotrytisC24, use = "p"))
...
length(names(athalDataH)[moduleColors=="blue"])
