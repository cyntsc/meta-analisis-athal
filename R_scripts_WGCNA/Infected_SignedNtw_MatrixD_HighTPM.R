...
### Date: June 10, 2021
### Latest update: xxxx 2021
###
### This is a PhD project associated at CICY Mx with the technical support (HPC) of 
###                                     Dr.Childs Lab at MSU.
###
### DATA ASSUMPIONS:
### 1) RNA-Seq libraries are                             ** 17  I N F E C T E D   SAMPLES  ** 
###                                                     OF A.THALIANA MATRIX TYPE D ( Only TPMs over the 99vo percentil)     
###
...
### 4) After that, resulting array were standarized under 3 criterias (specify in the thesis)
### 5) Resulting array from previous step was transformed to Log2(TPM+1)
...
athalDataHTPM <-  read.csv(here("data", "matrix_E_infectedHTPM.csv"), header=TRUE,  sep='\t')
dim(athalDataHTPM);   # 470  x  18
## Pull the names according with the size grp
SubGeneNames=names(athalDataHTPM);

## rearrange columns accordint to traits meta-data (must match)
col_order <- c("Genes", "Bc12",   "Bc12.1", "Bc18",   "Bc18.1", "Bc24", "Bc24.1", "Ch22",   "Ch22.1", "Ch22.2", "Ch22.3",
               "Ch40",   "Ch40.1", "Ch40.2", "Ch40.3", "Ss30", "Ss30.1", "Ss30.2")
athalDataHTPM <- athalDataHTPM[, col_order]
## You see that genes are listed in a column named "Genes" and samples are in columns
athalDataHTPM[1:5,]

## Manipulate athalDataHTPM so it matches the format WGCNA needs
row.names(athalDataHTPM) = athalDataHTPM$Genes   ## Pull the gene names 
#gene.names=rownames(athalDataHTPM)

## Transpose data to set as WGCNA needs
athalDataHTPM$Genes = NULL             ## Set genes column as index row
athalDataHTPM = as.data.frame(t(athalDataHTPM)) # transpose dataset, now samples are rows and genes are columns
#dim(athalDataHTPM) 
athalDataHTPM[1:10,1:5]
rownames(athalDataHTPM)   ## these are the sample names

## Run this to check if there are gene outliers
gsg = goodSamplesGenes(athalDataHTPM, verbose = 3)
gsg$allOK
...
if (!gsg$allOK)
   {if (sum(!gsg$goodGenes)>0)
       printFlush(paste("Removing genes:", paste(names(athalDataHTPM)[!gsg$goodGenes], collapse= ", ")));
       if (sum(!gsg$goodSamples)>0)
           printFlush(paste("Removing samples:", paste(rownames(athalDataHTPM)[!gsg$goodSamples], collapse=", ")))
       athalDataHTPM= athalDataHTPM[gsg$goodSamples, gsg$goodGenes]
       }
...
table(rownames(datTraits)==rownames(athalDataHTPM)) #should return TRUE if datasets align correctly, otherwise your names are out of order

#save(athalDataHTPM, datTraits, file="Athal_Infected_Log2TPM_SamplesAndTraits.RData")
...
sft=pickSoftThreshold(athalDataHTPM,
  dataIsExpr = TRUE,
...
plot(sft$fitIndices[,1], -sign(sft$fitIndices[,3])*sft$fitIndices[,2],xlab="Soft Threshold (power)",
     ...
     type="n", main = paste("Scale independence for Athal.infected hTPM \n (Matrix E)"));
