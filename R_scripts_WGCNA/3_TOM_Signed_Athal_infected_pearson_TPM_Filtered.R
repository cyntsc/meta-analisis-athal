...
# Load traits file
athalData3 <-  read.csv(here("data", "all_infected_TPM_filtered_new.csv"), header=TRUE,  sep='\t')
dim(athalData3);   # 13562  x  18
...
save(athalData3, datTraits, file="Athal_Infected_TPM_Filtered_SamplesAndTraits.RData")
#load("Athal_Infected_TPM_Filtered_SamplesAndTraits.RData")
...
plot(sft$fitIndices[,1], -sign(sft$fitIndices[,3])*sft$fitIndices[,2],xlab="Soft Threshold (power)",
     ylab="Scale Free Topology Model Fit, Signed R^2 (TPM Filtered)",
     type="n", main = paste("Scale independence for A.thaliana infected"));
text(sft$fitIndices[,1], -sign(sft$fitIndices[,3])*sft$fitIndices[,2],labels=powers,cex=cex1,col="red");
# Red line corresponds to using an R^2 cut-off
abline(h=0.80,col="red")

# Max correlation factor = 0.62744332
