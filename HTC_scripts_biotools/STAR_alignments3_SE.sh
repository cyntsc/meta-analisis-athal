#Cynthia-Soto Feb-2021

######################################################################
#
#                          DATASET3 AThaliana infected with fungi
...
echo > mylog_STAR_alignments3_SE.txt 
echo "Making  SAalignments with STAR/2.7.5a" >> mylog_STAR_alignments3_SE.txt
echo "Made by Cynthia.Soto" >> mylog_STAR_alignments3_SE.txt
echo "Date: feb 2, 2021" >> mylog_STAR_alignments3_SE.txt

mkdir athal_STAR_alignments3

for file in Dataset3_At_infect_trimmo/*.fq
do
	echo $file
	STAR --genomeDir athal_STAR_idx_genome/ --runThreadN 12 --readFilesIn $file --outFileNamePrefix athal_STAR_alignments3/$file -alignIntronMin 8 --alignIntronMax 25000 --outSAMtype BAM Unsorted #SortedByCoordinate
        echo "Executed job for file:" $file >> mylog_STAR_alignments3_SE.txt
done

echo "SAM/BAM Outputs on athal_STAR_alignments3 Directory"  >> mylog_STAR_alignments3_SE.txt
echo "My log bash file: mylog_STAR_alignments3_SE.txt " >> mylog_STAR_alignments3_SE.txt
