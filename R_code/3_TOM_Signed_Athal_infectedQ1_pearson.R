...
athalData3 <- read.csv(here("data", "all_infected_Log2_17ceros_Q1_drop.csv"), header=TRUE, sep='\t')
...
plot(sft$fitIndices[,1], -sign(sft$fitIndices[,3])*sft$fitIndices[,2],xlab="Soft Threshold (power)",
     ...
     type="n", main = paste("Scale independence. A.thal infected (Q1"));
text(sft$fitIndices[,1], -sign(sft$fitIndices[,3])*sft$fitIndices[,2],labels=powers,cex=cex1,col="red");

# Red line corresponds to using an R^2 cut-off
abline(h=0.531,col="red")
#abline(h=-0.90,col="blue")

# Mean connectivity as a function of the soft-thresholding power
plot(sft$fitIndices[,1], sft$fitIndices[,5],xlab="Soft Threshold (power)",ylab="Mean Connectivity", type="n",main = paste("Mean connectivity"))
text(sft$fitIndices[,1], sft$fitIndices[,5], labels=powers, cex=cex1,col="red")
abline(h=1671,col="blue");
...
plot(geneTree, xlab="Gene clusters", 
     main="Clustering tree based on the TOM dissimilarity measure of Athal infected (Q1)", 
...
plot(METree, main= "Clustering of module eigengenes for A thaliana infected (Q1)", 
     xlab= "", 
     sub= "")
...
plotDendroAndColors(geneTree, main= "Clustering of MEs merged on 0.2 for A thaliana infected (Q1)", 
...
labeledHeatmap(Matrix= moduleTraitCor,
               ...
               cex.text= 0.4,
               zlim= c(-1,1),
               main= paste("Module-trait relationships for A thaliana infected (Q1)"))
#INCLUE THE NEXT LINE TO SAVE TO FILE
#dev.off()
