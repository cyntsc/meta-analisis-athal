...
athalData2 <- read.csv(here("data", "all_log2_tidy.csv"), header=TRUE, row.names='Genes', sep='\t')
dim(athalData2);
names(athalData2);
## Some useless columns and 2 samples were removed from the dataset due their low quality 
athalData2 = subset(athalData2, select = -c(Ss30,Ss30.1)); # 24326 genes (rows) x 19 samples (cols)
dim(athalData2);
athalData2[1:18];
class(athalData2);  #The object is a data.frame

## WGCNA requires genes be given in the columns
gene.names=rownames(athalData2);
gene.names;
trans.athalData2=t(athalData2);

## Focus in a small group of genes to test the function
##n=25000;
##datExpr=trans.athalData2[,1:n];

datExpr=trans.athalData2
dim(datExpr)
...
plot(sft$fitIndices[,1], -sign(sft$fitIndices[,3])*sft$fitIndices[,2],xlab="Soft Threshold (power)",ylab="Scale Free Topology Model Fit, signed R^2",type="n", main = paste("Scale independence. 18 samples."));
